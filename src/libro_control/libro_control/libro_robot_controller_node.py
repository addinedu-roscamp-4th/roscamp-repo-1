...
from std_msgs.msg import Int32, String, Float32MultiArray, Bool, Float32
...
class LibroControllerNode(Node):
    def __init__(self):
        ...
        self.pinky_lcd_pub = self.create_publisher(Bool, f'{self.robot_id}/nav_reached', 10)                # 핑키 목적지 도착시 LCD 표시 요청 발행
        self.libro_battery_pub = self.create_publisher(Float32, f'{self.robot_id}/battery_present', 10)     # 로봇 배터리 정보 발행
        ...
        self.battery_sub = self.create_subscription(
            Float32, f'{self.robot_id}/pinky_battery_present', self.battery_callback, 10)       # 로봇 배터리 정보 구독
    ...
    def publish_check_cabinet(self):
        msg = String()
        msg.data = f'Pickup Place : {self.pickup_place_name}, Cabinet : {self.target_cabinet}'
    ...
    def publish_stop_robot(self):
        if self.last_control_state == 1 or self.last_control_state == 5 or self.last_control_state == 12 or self.last_control_state == 14:
            msg = Bool()
            msg.data = True
    ...
    def goal_place_callback(self, msg):
        ...
        if self.control_state == 14 and msg.success:
            ...
            self.pinky_lcd_pub.publish(goal_reached_msg)
    ...
    def battery_callback(self, msg):
        self.libro_battery = msg.data
        # self.get_logger().info(f'Robot : {self.robot_id} received battery: {self.libro_battery}')
        # Float32 메시지 생성 및 발행
        battery_msg = Float32()
        battery_msg.data = self.libro_battery
        self.libro_battery_pub.publish(battery_msg)
