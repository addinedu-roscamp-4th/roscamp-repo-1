...
class BookCheckPublisher(Node):
    def __init__(self):
        super().__init__('book_check_publisher')
        self.publisher_ = self.create_publisher(String, 'book_check', 10)
        timer_period = 0.5
...
def main(args=None):
    ...
    try:
        book_check_publisher = BookCheckPublisher()
        rclpy.spin(book_check_publisher)
    except KeyboardInterrupt:
        pass
    except Exception as e:
        print(f'Error: {str(e)}')
    finally:
        if 'book_check_publisher' in locals():
            book_check_publisher.destroy_node()
        rclpy.shutdown()
