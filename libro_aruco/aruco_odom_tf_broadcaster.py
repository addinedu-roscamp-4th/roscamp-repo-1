...
class ArucoOdomTfBroadcaster(Node):
    def __init__(self):
        ...
        # LPF 파라미터
        self.declare_parameter('lpf_alpha_linear', 0.1)
        self.declare_parameter('lpf_alpha_angular', 0.1)

        # 공분산 파라미터
        self.declare_parameter('pose_cov_x', 0.01)
        self.declare_parameter('pose_cov_y', 0.01)
        self.declare_parameter('pose_cov_z', 1e9)
        self.declare_parameter('pose_cov_roll', 1e9)
        self.declare_parameter('pose_cov_pitch', 1e9)
        self.declare_parameter('pose_cov_yaw', 0.005)
        self.declare_parameter('twist_cov_vx', 0.1)
        self.declare_parameter('twist_cov_vy', 0.1)
        self.declare_parameter('twist_cov_vz', 1e9)
        self.declare_parameter('twist_cov_wx', 1e9)
        self.declare_parameter('twist_cov_wy', 1e9)
    ...
    def normalize_quaternion_array(self, quat_array):
        """쿼터니언 배열 정규화 [x, y, z, w]"""
    ...
    def normalize_quaternion_tf_msg(self, quat_msg):
        """TransformStamped의 쿼터니언 정규화"""
        quat_array = np.array([quat_msg.x, quat_msg.y, quat_msg.z, quat_msg.w])
        normalized = self.normalize_quaternion_array(quat_array)
        quat_msg.x = normalized[0]
        quat_msg.y = normalized[1]
        quat_msg.z = normalized[2]
        quat_msg.w = normalized[3]
        return quat_msg
    ...
    def map_pose_callback(self, msg: PoseStamped):
        ...
        if self.initial_marker_pose_in_map is None:
            ...
            self.initial_map_pose_to_odom_tf.transform.translation.z = self.initial_marker_pose_in_map.position.z
            ...
            # 쿼터니언 정규화
            initial_q = self.normalize_quaternion_array(np.array(initial_q))
            ...
            # 정규화된 쿼터니언 적용
            q_odom_in_map = self.normalize_quaternion_array(np.array(q_odom_in_map))
        ...
        # static TF가 아직 발행되지 않았으면, 나머지 로직 수행 안 함
        if not self.static_tf_published:
            self.get_logger().info("Waiting for initial ArUco marker pose to publish static TF...",
        ...
        # 쿼터니언 정규화
        q_map_odom = self.normalize_quaternion_array(np.array(q_map_odom))
        ...
        # 쿼터니언 정규화
        q_map_current_marker = self.normalize_quaternion_array(np.array(q_map_current_marker))
        ...
        # 쿼터니언 정규화
        q_odom_base = self.normalize_quaternion_array(np.array(q_odom_base))

        # 3. Odometry 메시지 발행
        odom_msg = Odometry()
        odom_msg.header.stamp = current_time_msg
        odom_msg.header.frame_id = self.robot_odom_frame_id
        ...
        odom_msg.pose.pose.position.z = trans_odom_base[2]
        ...
        if dt > 1e-6 and self.previous_marker_pose_in_map is not None:
            # map 기준 위치 변화량 계산
            ...
            # 쿼터니언 정규화
            q_prev_map = self.normalize_quaternion_array(np.array(q_prev_map))
            q_curr_map = self.normalize_quaternion_array(np.array(q_curr_map))

            delta_q_map = tf_transformations.quaternion_multiply(q_curr_map,
                                                                 tf_transformations.quaternion_inverse(q_prev_map))
            delta_q_map = self.normalize_quaternion_array(np.array(delta_q_map))

            _, _, dyaw_map = tf_transformations.euler_from_quaternion(delta_q_map)

            # map 기준 속도를 로봇의 현재 base_footprint 프레임 기준으로 변환
            _, _, current_robot_yaw_map = tf_transformations.euler_from_quaternion(q_curr_map)

            linear_velocity_x_raw = (dx_map * np.cos(-current_robot_yaw_map) - dy_map * np.sin(
                -current_robot_yaw_map)) / dt
            linear_velocity_y_raw = (dx_map * np.sin(-current_robot_yaw_map) + dy_map * np.cos(
                -current_robot_yaw_map)) / dt
            angular_velocity_z_raw = dyaw_map / dt
        ...
        odom_msg.twist.twist.linear.y = filtered_vy
        ...
        # 최종 쿼터니언 정규화
        t_odom_to_base.transform.rotation = self.normalize_quaternion_tf_msg(t_odom_to_base.transform.rotation)
