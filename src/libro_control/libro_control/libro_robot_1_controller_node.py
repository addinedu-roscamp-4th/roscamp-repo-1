...
class LibroControllerNode(Node):
    def __init__(self):
        ...
        self.robot_name_id = 'libro_robot_1'  # 로봇 고유 이름/ID  'libro_robot_1', 'libro_robot_2' 까지만 지원
        ...
        self.state_pub = self.create_publisher(Int32, 'controller_1_mission_state', 10)
        self.libro_robot_state_pub = self.create_publisher(String, 'libro_robot_1_pickup_state', 10)
        self.libro_robot_log_pub = self.create_publisher(LibroRobotLog, 'libro_robot_log', 10)
        self.robot1_mission_complete_pub = self.create_publisher(Bool, 'libro_robot_1_mission_complete', 10)
        ...
        self.mission_response_sub = self.create_subscription(
            PinkyMissionResponse, 'libro_mobile_goal_response', self.mission_response_callback, 10)
    ...
    def counter_check_response_callback(self, msg):
        if self.control_mission_state == 8 and msg.data == "ON":
            ...
            # Create and publish Bool message
            complete_msg = Bool()
            complete_msg.data = True
            self.robot1_mission_complete_pub.publish(complete_msg)
