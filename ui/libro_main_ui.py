...
from PyQt5.QtCore import QTimer, Qt, pyqtSignal, QPoint, QSettings, QRegExp, QSignalBlocker, QTimer, Qt, pyqtSignal, QPoint, QSettings, QRegExp, QSignalBlocker, pyqtSignal, QAbstractTableModel, QDateTime
...
class LibroAdmin(QMainWindow, main_ui): 
    ...
    def __init__(self):
        ...
        self.start_dateTimeEdit.setDateTime(QDateTime.currentDateTime())
...
###################################################################################    
    ...
    def current_erorr2_update(self,message_2):
        ...
        self.libro2_activate_sw.setEnabled(False)
        blocker = QSignalBlocker(self.libro2_activate_sw)
        self.libro2_activate_sw.setChecked(True)
        self.toggle = 1
    ...
    def current_erorr3_update(self, message_3):
        ...
        self.libro3_activate_sw.setEnabled(False)
...
#log 테이블 데이터및 스타일 시트
    def robot_log(self, log_list):
        ...
        self.new_list = []
        for row in log_list:
            dt_str = row[0].split('.')[0]  # '2025-05-23T19:59:54' 형태로 자름
    ...
    def category_search_box(self):
        self.setupUi(self)
        self.category_search.addItems(["시간", "로봇ID", "주문번호", "로봇로그", "성공/실패"])
        self.category_search.currentTextChanged.connect(self.on_category_changed)
