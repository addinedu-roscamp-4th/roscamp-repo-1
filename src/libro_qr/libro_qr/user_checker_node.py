...
class UserCheckerNode(Node):
    def __init__(self):
        ...
        # Create subscriber for nav_reached
        self.nav_reached_subscription = self.create_subscription(
            Bool,
            f'{self.robot_id}/nav_reached',
            self.nav_reached_callback,
            10)
    ...
    def emergency_stop_callback(self, msg):
        ...
        if msg.data:  # If emergency stop is True
            self.get_logger().info('Emergency stop activated')
            
            # Stop the basic emotion display
            self.display_active = False
            time.sleep(0.2)  # Give time for basic display to stop
            
            try:
                # Show stop sign image
                stop_sign_path = os.path.join(self.emotion_path, "stop_sign.jpeg")
                if os.path.exists(stop_sign_path):
                    img = Image.open(stop_sign_path)
                    self.lcd.img_show(img)
                    time.sleep(5)  # Display for 5 seconds
                else:
                    self.get_logger().error(f'Stop sign image not found at: {stop_sign_path}')
                    # Fallback to text message if image not found
                    self.show_text_message("EMERGENCY STOP!", 5)
            except Exception as e:
                self.get_logger().error(f'Error displaying stop sign: {str(e)}')
                self.show_text_message("EMERGENCY STOP!", 5)
            finally:
                # Resume basic emotion display
                self.display_active = True
    ...
    def nav_reached_callback(self, msg):
        """Callback function when navigation goal is reached"""
        if msg.data:  # If navigation goal is reached
            ...
            try:
                ...
                self.show_text_message("Goal Reached! ^^", duration=5)
            except Exception as e:
                self.get_logger().error(f'Error displaying goal reached message: {str(e)}')
            finally:
                # Resume basic emotion display
                self.display_active = True
                if not self.display_thread.is_alive():
