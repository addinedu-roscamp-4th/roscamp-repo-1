...
class LibroTaskManagerNode(Node):
    def __init__(self):
        ...
        # Mission completion subscribers - 동적으로 관리
        self.mission_completion_subs = {}
    ...
    def active_robots_callback(self, msg):
        ...
        for robot_id in current_robots:
            ...
            if robot_name not in self.robot_states:
                self.robot_states[robot_name] = RobotState.WAITING
                self.current_tasks[robot_name] = None
                # 미션 완료 구독자 생성
                self.create_mission_completion_subscriber(robot_name)
        ...
        for robot_name in list(self.robot_states.keys()):
            ...
            if robot_id not in current_robots:
                # 미션 완료 구독자 제거
                if robot_name in self.mission_completion_subs:
                    self.destroy_subscription(self.mission_completion_subs[robot_name])
                    del self.mission_completion_subs[robot_name]
    ...
    def create_mission_completion_subscriber(self, robot_name):
        """로봇별 미션 완료 구독자 생성"""
        topic_name = f'{robot_name}/completed'
        self.mission_completion_subs[robot_name] = self.create_subscription(
            Bool,
            topic_name,
            lambda msg: self.mission_completion_callback(robot_name, msg),
            10
        )
        self.get_logger().info(f'Created mission completion subscriber for {robot_name}')

    def mission_completion_callback(self, robot_name, msg):
        """미션 완료 콜백"""
        if msg.data:  # 미션이 완료되었다면
            self.get_logger().info(f'{robot_name} completed its mission')
            
            # 현재 상태가 미션 중인지 확인
            current_state = self.robot_states[robot_name]
            if current_state in [RobotState.PICKUP_MISSION, RobotState.NAVIGATION_MISSION]:
                # 상태를 WAITING으로 변경
                self.update_robot_state(robot_name, RobotState.WAITING)
                # 현재 태스크 초기화
                self.current_tasks[robot_name] = None
                ...
                self.get_logger().warn(f'{robot_name} received completion signal but was not in mission state')
    ...
    def process_tasks(self):
        ...
        for robot_name in list(self.robot_states.keys()):
            if (self.robot_states[robot_name] == RobotState.WAITING and 
                not self.current_tasks[robot_name] and 
                self.service_requests):
                ...
                for i, request in enumerate(self.service_requests):
                    if request['type'] == 'book_request':
                        ...
                        msg.pickup_place = self.create_mission_place(request['pickup_place'][0], robot_name)
                        
                        self.mission_pickup_pub.publish(msg)
                        self.current_tasks[robot_name] = request
                        self.update_robot_state(robot_name, RobotState.PICKUP_MISSION)
                        
                        # Remove the assigned request from the list
                        self.service_requests.pop(i)
                        self.save_service_requests()
                        
                        self.get_logger().info(f'Assigned book pickup task to {robot_name}: {request["book_1"][0]["book_name"]}' + 
                        ...
                        msg.order_number = request['order_number']
                        msg.start_place = self.create_mission_place(request['start_place'][0], robot_name)
                        msg.end_place = self.create_mission_place(request['end_place'][0], robot_name)
                        
                        self.mission_nav_pub.publish(msg)
                        self.current_tasks[robot_name] = request
                        self.update_robot_state(robot_name, RobotState.NAVIGATION_MISSION)
                        
                        # Remove the assigned request from the list
                        self.service_requests.pop(i)
                        self.save_service_requests()
                        
                        self.get_logger().info(f'Assigned navigation task to {robot_name}: {request["start_place"][0]["name"]} -> {request["end_place"][0]["name"]}')
                        break
